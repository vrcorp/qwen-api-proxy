...
> **Быстрое развертывание:** Нажмите на кнопку выше для развертывания за несколько минут. См. [DEPLOYMENT.md](DEPLOYMENT.md) для подробных инструкций по всем платформам.

## ✨ Основные возможности

- **🚀 Готовность к Serverless**: Полностью совместимо с Railway, Render, AWS Lambda, Google Cloud Run, Vercel и другими облачными платформами
- **📦 Без дисковых операций**: Все файлы обрабатываются в памяти, не требуется постоянное хранилище
- **⚙️ Конфигурация через переменные окружения**: Быстрая настройка без редактирования файлов
...
- **📂 Загрузка файлов**: Загрузка изображений через прокси на Qwen CDN (в памяти)
...
# Токены Qwen (рекомендуется для serverless)
QWEN_TOKEN=your-token-here
# или несколько токенов:
QWEN_TOKENS=token1,token2,token3

# API ключи для авторизации прокси (рекомендуется для serverless)
API_KEYS=key1,key2,key3

# Пропустить интерактивное меню
SKIP_ACCOUNT_MENU=false
```

> **💡 Совет:** Для облачных платформ (Railway, Render, AWS Lambda) используйте переменные окружения вместо файлов. См. [DEPLOYMENT.md](DEPLOYMENT.md) для подробностей.
...
Здесь вы можете тестировать все эндпоинты API в интерактивном режиме.

### 1.5 Варианты развертывания

Приложение готово к развертыванию на различных платформах:

- **Railway** / **Render** - быстрое развертывание одной командой
- **AWS Lambda** - serverless с автомасштабированием
- **Google Cloud Run** - контейнеры с оплатой за использование
- **Vercel** - требует адаптации к Edge Runtime
- **Cloudflare Workers** - требует замены Express на Hono
- **Docker** - портативные контейнеры для любой платформы
...
## 2. Архитектура и совместимость с Serverless

### 🚀 Нулевая зависимость от файловой системы

Приложение **полностью совместимо с serverless** платформами, потому что:

1. **Логирование** - только в консоль (stdout/stderr), без файлов журналов
2. **Загрузка файлов** - обработка в памяти, временные файлы не создаются
3. **Конфигурация** - через переменные окружения, чтение файлов опционально
4. **Хранение токенов** - приоритет переменным окружения, файлы используются как fallback

### 📦 Целевые платформы

Работает на:

- ✅ **Railway, Render, Heroku** - PaaS платформы
- ✅ **AWS Lambda, Google Cloud Run** - serverless контейнеры
- ✅ **Docker** - стандартная контейнеризация
- ⚠️ **Vercel, Cloudflare Workers** - требуется адаптация (см. DEPLOYMENT.md)

Подробнее см. [ARCHITECTURE.md](ARCHITECTURE.md) и [DEPLOYMENT.md](DEPLOYMENT.md).

---

## 3. Авторизация через API-ключи

> ⚠️ **Новое:** Для совместимости с serverless рекомендуется использовать переменную окружения `API_KEYS`.

### Рекомендуемый подход (переменная окружения)

Добавьте в `.env`:

```bash
API_KEYS=my-secret-key-1,my-secret-key-2,admin-key-xyz
```

Или установите переменную окружения в вашей облачной платформе.

### Устаревший подход (файл Authorization.txt)
...
2. **Отключить авторизацию** – оставьте файл пустым и не задавайте `API_KEYS`. Middleware пропустит все запросы.
...
### Рекомендуемый подход (переменные окружения)

Для serverless развертывания используйте переменные окружения:

```bash
# Один токен
QWEN_TOKEN=your-qwen-token-here

# Или несколько токенов для автоматической ротации
QWEN_TOKENS=token1,token2,token3
```

Приложение автоматически загрузит токены из окружения при запуске.

### Устаревший подход (управление аккаунтами через меню)
...
Подробную информацию по конкретным темам см.:

- **[Руководство по развертыванию](DEPLOYMENT.md)** - Полное руководство по развертыванию на Railway, Render, AWS Lambda, Google Cloud Run, Vercel, Cloudflare Workers и Docker
- **[Документация по архитектуре](ARCHITECTURE.md)** - Техническая архитектура, дизайн компонентов, потоки запросов, модель безопасности и соображения масштабируемости
- **[Руководство по генерации изображений и видео](IMAGE_VIDEO_GENERATION_GUIDE.md)** - Подробное руководство по функциям генерации text-to-image (t2i) и text-to-video (t2v)
- **[Интерактивная документация API](http://localhost:3264/api-docs)** - Swagger UI для тестирования эндпоинтов (доступно при запущенном сервере)
